...
@st.cache_resource(show_spinner=False) #rerank모델 받아오는 걸 한번만 하고 그 뒤엔 캐시에 저장된걸로 이용
...
# 사용자 질문을 다시 쓰는 함수 -> OpenAI API 비용 발생(gpt-3.5 쓰도록 고정정)
...
# Upstage Fact checker 함수 -> UpStage API 비용 발생
...
def run_rag_pipeline(
    ...
    rag_chain = (
        ...
        | llm
        | StrOutputParser()
    ).with_config({"callbacks": [TRACER]})                      # callback 추적 되게끔 추가한 부분
                     ...
                     try:
                         ...
                         if use_gpt_scoring_toggle and eval_docs_used:
                             with st.spinner(f"[{run_id}] GPT Scoring 중... (OpenAI API)"):
                                 ...
                                 ########여기다가 평가셋 불러와서 gorund truth, question, eval_response 넣는거 진행하면 되겠다.
                                 ###query 부분 안고장나게 유의.
